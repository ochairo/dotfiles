...
_core_map_lib() {
	...
	env) echo "$CORE_DIR/constants.sh" ;;
	os) echo "$CORE_DIR/os.sh" ;;
	selection) echo "$CORE_DIR/registry.sh" ;;
	...
	update) echo "$CORE_DIR/install_helpers.sh" ;;
	term) echo "$CORE_DIR/term.sh" ;;
	util) echo "$CORE_DIR/util.sh" ;;
	validation) echo "$CORE_DIR/validation.sh" ;;
	dependency) echo "$CORE_DIR/dependency.sh" ;;
	error) echo "$CORE_DIR/error.sh" ;;
	*) return 1 ;;
	esac
}
