...
# Validate install script exists and is executable
validate_component_install_script() {
    local component_name="$1"
    local install_script="$COMPONENTS_DIR/$component_name/install.sh"
    local errors=0

    if [[ ! -f "$install_script" ]]; then
        log_error "Component $component_name: missing install.sh script"
        ((errors++))
    elif [[ ! -x "$install_script" ]]; then
        log_warn "Component $component_name: install.sh is not executable"
    fi
