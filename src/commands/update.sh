...
#!/usr/bin/env bash
# usage: dot update [--pull]
# summary: Show or fetch upstream changes (delegates to legacy update-check.sh)
# group: core
set -euo pipefail
PROJECT_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
LEGACY="$PROJECT_ROOT/update-check.sh"
if [ ! -f "$LEGACY" ]; then
	echo "Legacy update-check.sh missing" >&2
	exit 1
fi
exec "$LEGACY" "$@"
