#!/usr/bin/env bash
# Zellij post-install script: links configuration directory
set -euo pipefail
...
CONFIG_SOURCE="$(config_resolve_dir zellij || true)"
TARGET_DIR="${HOME}/.config/zellij"

if [[ -n "$CONFIG_SOURCE" && -d "$CONFIG_SOURCE" ]]; then
  fs_symlink "$CONFIG_SOURCE" "$TARGET_DIR" "zellij" || { msg_error "Failed to link Zellij config"; return 1; }
  msg_success "Zellij configuration linked"
else
  msg_warn "Zellij config directory not found (checked standard paths)"
fi
